...
func open_from_map() -> void:
	...
	hotkey_lbl.text = "💡 Tab = fermer  |  Ctrl+Shift+X = +9999 XP (test)"
	close_btn.text  = "Fermer  [Tab]"
	visible = true

# ── Cheat code Ctrl+Shift+X (géré ici car uniquement utile quand visible) ─────
func _unhandled_key_input(event: InputEvent) -> void:
	if not visible:
		return
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == KEY_X and event.ctrl_pressed and event.shift_pressed:
			Global.add_xp(9999)
			_refresh_all_cards()
			_refresh_xp_bar()
