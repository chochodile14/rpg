...
# ── Bob de marche ─────────────────────────────────────────────────────────────
@export var bob_amplitude : float = 2.5   # hauteur du bob en pixels
@export var bob_frequency : float = 8.0   # cycles/seconde
...
func _physics_process(delta: float) -> void:
	...
	if moving_now:
		_bob_timer += delta * bob_frequency * TAU
		...
		if _was_moving:
			_bob_timer = 0.0
			_was_moving = false
		# Retour doux à la position neutre à l'arrêt
		animated_sprite_2d.position.y = lerp(animated_sprite_2d.position.y, _base_y, delta * 15.0)
